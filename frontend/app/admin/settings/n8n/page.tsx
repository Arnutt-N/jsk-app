...
export default function N8nSettingsPage() {
    ...
    const fetchConfig = useCallback(async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                logger.error('Failed to fetch n8n config', { status: res.status });
                setError(msg);
            }
        } catch (err) {
    ...
    const handleSave = async () => {
        setProcessing('SAVE');
        try {
            const payload: Record<string, string> = { webhook_url: form.webhook_url };
            ...
            logger.error('Failed to save n8n config', err);
    ...
    if (loading) return <LoadingSpinner label="Loading n8n settings..." />;
        ...
        <div className="thai-text space-y-6 animate-in fade-in duration-500">
            ...
            <div className="bg-surface rounded-2xl border border-border-default shadow-sm overflow-hidden">
                <div className="flex justify-between items-center p-6 border-b border-border-default bg-bg">
                    <h2 className="font-bold text-text-primary">Webhook Configuration</h2>
                    {!isEditing && (
                        <Button
                            variant="ghost"
                            size="sm"
                            ...
                            onClick={() => {
                                setForm({ webhook_url: config?.webhook_url || '', api_key: '' });
                                setIsEditing(true);
                            }}
                        >
                            แก้ไข
            ...
            <Modal
                ...
                title={testResult?.success ? 'Webhook Test Passed' : 'Webhook Test Failed'}
                maxWidth="sm"
            >
            ...
            >
                <div className="flex flex-col items-center text-center p-4 pt-2">
                    ...
                    <p className="text-text-secondary mb-6">
                        บันทึกข้อมูล n8n Webhook เรียบร้อยแล้ว
                    </p>
