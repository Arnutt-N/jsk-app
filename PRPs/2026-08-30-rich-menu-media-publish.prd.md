...
**Created:** 2026-08-30
**Status:** Approved for implementation (reviewed with user 2026-08-30)
...
- **AC-1.3** A rich-menu uploaded before this fix (local dev) still shows its
  preview after migration (backfill). Note: `backend/uploads/rich_menus/` is
  currently empty locally, so the local backfill is a no-op today; prod rows
  with missing files (Koyeb ephemeral FS) stay null per Decision 2.
...
- **AC-2.3** Non-PNG/JPEG uploads are rejected 422 (the frontend `accept`
  attribute already promises this whitelist).
...
- **AC-3.2** List-page badge and action buttons derive from real state: "Set
  ...
  a `FAILED` sync shows a "Re-sync" affordance with the error.
...
- **AC-4.1** Menu missing/stale on LINE (verified via `get_from_line`) → **409**
  ...
  `sync_status=FAILED, last_sync_error`.
...
### In scope
- `rich_menus` schema change: `image_media_id` FK → `media_files`, drop
  `image_path`, best-effort backfill migration
- Backend endpoints: upload / sync / publish / delete / list / get-single
- Frontend: list + edit pages (`image_url`, sync-failure surfacing, button
  gating, toast handling)
- Tests: backend (endpoint + service + migration round-trip), frontend vitest
...
- Updating stale agent-facing docs that reference `image_path`:
  ...
  `references/menu_reference.md`, `references/rich_menu_frontend_reference.md`)
  and `.claude/PRPs/prds/rich-menu-image-generator.prd.md:92,149` (its
  `image_path LIKE '%generated_%'` metrics die with the column) — tracked
  separately so this PR stays code-focused

## 5. Rollback

- Code: revert the merged PR (no new env flags).
- Migration: `alembic downgrade -1` — downgrade writes files back to disk
  best-effort and restores `image_path`.

## 6. Decisions from review

1. `image_url` is relative `/api/v1/media/{id}` — consistent with LIFF and
   admin-files usage. ✅
2. Menus whose disk files are already gone (Koyeb) will show "No Image" and need
   one re-upload via the edit page; local dev rows are backfilled by the
   migration. ✅ accepted
3. Drop `image_path` in the same PR (single source of truth); rollback is the
   downgrade path. ✅ accepted