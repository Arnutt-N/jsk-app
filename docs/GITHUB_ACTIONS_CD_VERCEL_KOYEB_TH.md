...
## Seed Admin User (ทำครั้งแรกหลัง deploy)

workflow นี้รัน database migration ให้อัตโนมัติ แต่ไม่ได้ seed admin user

สำหรับ deploy ครั้งแรก ต้องรัน seed script จาก local machine ที่ต่อ production DB:

```bash
cd backend
export DATABASE_URL=ค่าจาก Supabase
export SECRET_KEY=ค่าเดียวกับ production
export ADMIN_DEFAULT_PASSWORD=รหัสที่ต้องการ
python scripts/seed_admin.py --apply
```

ดูรายละเอียดเพิ่มเติมที่ [DEPLOY_VERCEL_KOYEB_SUPABASE_UPSTASH_TH.md](./DEPLOY_VERCEL_KOYEB_SUPABASE_UPSTASH_TH.md) Step 4.6
...
- ยังไม่ได้ seed admin user อัตโนมัติ (ต้องทำเองครั้งแรก)
